...
class MyHTMLParser(HTMLParser):
   ...
   """
    ...
    def get_words(self):
        """
        Returns a list of the words in the ebook
        """
        return(self._words)
...
def parse_page():
    """
    Parses a webpage and searches for an ebook.
    If found it counts the frequency of words in the ebook
    ...
    try:
        ...
        words_str = parser.get_words()
       
        # use an regular expression to find the title of the ebook
        match = re.search(TITLE_PATTERN, words_str)
        if match:
            #found the title of the book
            title = match.group(1).strip()
        
        #search just the book, not text before and after the book text
        start_index = words_str.find("*** start of the project gutenberg ebook")
        end_index = words_str.find("*** end of the project gutenberg ebook")
        if start_index != -1 and end_index != -1:
            sub_data = words_str[start_index+40: end_index]
        else:
            sub_data = words_str
